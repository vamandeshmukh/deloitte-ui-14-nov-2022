...
const saveCustDetails = () => {
    const newCustomer = new Customer();
    newCustomer.customerId = document.getElementById('customerId').value;
    registeredCustomers[registeredCustomers.length] = newCustomer;
    document.getElementById('custDataForm').reset();
    // document.getElementById('custData').innerHTML = `${newCustomer.customerId} ${newCustomer.firstName} ${newCustomer.phones[0]} ${newCustomer.email} ${newCustomer.address.city}`;
    console.log(newCustomer.customerId);
    console.log(registeredCustomers.length);
